...
window.addEventListener("load", function () {
    // sidebar expanble
    const expandableElement = document.querySelector("aside.exapandable");
    if (expandableElement) {
        expandableElement.addEventListener("mousedown", isSidebarExpandStart);
        expandableElement.addEventListener("touchstart", isSidebarExpandStart);
    }
    ...
    // responsive sidebar
    const toggleBtn = document.querySelector("button.sidebar-toggle-btn");
    if (toggleBtn) {
    ...
    const mainArticle = this.document.querySelector("article.main");
    if (mainArticle) {
        mainArticle.addEventListener("click", hideSidebar);
        mainArticle.addEventListener("touchstart", hideSidebar);
        mainArticle.addEventListener("mousedown", hideSidebar);
    }
    ...
    this.window.addEventListener("resize", () => {
        if (window.innerWidth > 900) {
            ...
            const toggleBtnResize = document.querySelector("button.sidebar-toggle-btn");
            if (toggleBtnResize) {
                toggleBtnResize.setAttribute("aria-expanded", "false");
            }
        }
    });
